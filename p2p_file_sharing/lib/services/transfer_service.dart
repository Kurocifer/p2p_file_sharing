...
class TransferService {
  ...
  Future<void> uploadFile(String peerIP, int peerPort, String fileName) async {
    ...
    await for (final chunk in fileStream) {
      ...
      logger.logMessage(
        message: "[INFO] Sent chunk $sequenceNumber of size ${chunk.length} bytes.",
      );
      sequenceNumber++;
    }
...
Future<void> downloadFile() async {
  ...
  logger.logMessage(message: '[INFO] Listening for file requests on port $port');
      ...
      try {
        ...
        if (request['requestType'] == 'download') {
          ...
          if (availableFiles.contains(fileName)) {
            ...
            await uploadFile(datagram.address.address, datagram.port, '$sharedFolderPath/$fileName');
          } else {
            logger.logMessage(message: '[ERROR] Requested file $fileName not found.');
          }
        }
      } catch (e) {
...
List<String> _getAvailableFiles(String directoryPath) {
  try {
    ...
    if (directory.existsSync()) {
      ...
      logger.logMessage(message: '[WARNING] Shared directory does not exist: $directoryPath');
      return [];
    }
  } catch (e) {
    logger.logMessage(message: '[ERROR] Failed to list files in directory: $e');
    return [];
  }
}

  /// Directory-specific features
  Future<void> sendDirectoryStructure(String peerIP, int peerPort, String directoryPath) async {
    final udpSocket = await UDP.bind(Endpoint.any());
    ...
    logger.logMessage(message: '[INFO] Sent directory structure to $peerIP:$peerPort.');
    udpSocket.close();
  }
  ...
  void listenForDirectoryRequests(int port, String directoryPath) async {
    ...
    logger.logMessage(message: '[INFO] Listening for directory requests on port $port.');
  ...
  Future<void> requestDirectoryStructure(String peerIP, int peerPort, Function(Map<String, dynamic>) onSuccess) async {
    final udpSocket = await UDP.bind(Endpoint.any());
    ...
    logger.logMessage(message: '[INFO] Sent directory structure request to $peerIP:$peerPort.');
